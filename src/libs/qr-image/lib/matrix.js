...
export default class Matrix {
  static getMatrix(data) {
    var matrix = this.init(data.version);
    ...
    for (var mask = 0; mask < 8; mask++) {
      ...
      this.fillReserved(matrix, data.ec_level, mask);
      var p = this.calculatePenalty(matrix);
      if (p < penalty) {
        penalty = p;
        bestMask = mask;
      }
    ...
    this.fillReserved(matrix, data.ec_level, bestMask);
  ...
  static fillAlignAndTiming(matrix) {
    var N = matrix.length;
    if (N > 21) {
      var len = N - 13;
  ...
  static fillReserved(matrix, ec_level, mask) {
    var N = matrix.length;
  ...
  static fillData(matrix, data, mask) {
    var N = matrix.length;
    var row, col, dir = -1;
    row = col = N - 1;
  ...
  static calculatePenalty(matrix) {
    var N = matrix.length;
    var penalty = 0;
    // Rule 1
    for (var i = 0; i < N; i++) {
