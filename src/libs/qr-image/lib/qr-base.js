...
export default class QRBase {
  static matrix(text, ec_level, version, parse_url) {
    ec_level = EC_LEVELS.indexOf(ec_level) > -1 ? ec_level : 'M';
    var message = encode(text, parse_url);
    var data = this.fillTemplate(message, this.getTemplate(message, ec_level, version));
    return matrix.getMatrix(data);
  }

  static getTemplate(message, ec_level, preferedVersion) {
    var i = 1;
    var len;

    if (preferedVersion && preferedVersion > 0 && preferedVersion < 41) {
      len = Math.ceil(message.data27.length / 8);
      for (/* i */; i < 41; i++) {
        let version = versions[preferedVersion][ec_level];
  ...
  static fillTemplate(message, template) {
